...
def test_distinct_vectors_all_kept():
    ...
    result = greedy_dedup(emb, tau=0.94)
...
def test_tau_is_a_strict_keep_boundary():
    ...
    sim = float(a @ b)
    at_tau = greedy_dedup(np.stack([a, b]), tau=sim)
    assert at_tau.kept == [0]  # exactly at tau: absorbed
    below_tau = greedy_dedup(np.stack([a, b]), tau=np.nextafter(sim, 1.0))
...
def test_rejects_bad_shape():
    with pytest.raises(ValueError):
        greedy_dedup(np.zeros(5, dtype=np.float32), tau=0.94)


def test_empty_input():
    result = greedy_dedup(np.zeros((0, 8), dtype=np.float32), tau=0.94)
    assert result.kept == []
